...
hSharedMemoryDll = LoadLibrary(TEXT("SharedMemory.dll"));
			/////////////////////////////////////////////////////////////////////////////////////////////////////////////
			// get pointers to these functions in the dll
			/////////////////////////////////////////////////////////////////////////////////////////////////////////////
			//	bool LockSharedMemory(HANDLE sharedDataHandle, unsigned int timeoutInMilliseconds)
			...
			//if ( hSharedMemoryDll != NULL ) {
				lpfnLockSharedMemory = GetProcAddress(HMODULE (hSharedMemoryDll),"LockSharedMemory");
				lpfnUnlockSharedMemory = GetProcAddress(HMODULE (hSharedMemoryDll),"UnlockSharedMemory");
				lpfnWriteSharedMemory = GetProcAddress(HMODULE (hSharedMemoryDll),"WriteSharedMemory");
				lpfnReadSharedMemory = GetProcAddress(HMODULE (hSharedMemoryDll),"ReadSharedMemory");
				lpfnDestroySharedMemory = GetProcAddress(HMODULE (hSharedMemoryDll),"DestroySharedMemory");
				lpfnCreateSharedMemory = GetProcAddress(HMODULE (hSharedMemoryDll),"CreateSharedMemory");
			
				LockSharedMemory = (pLockSharedMemoryFUNC)lpfnLockSharedMemory;
				UnlockSharedMemory = (pUnlockSharedMemoryFUNC)lpfnUnlockSharedMemory;
				WriteSharedMemory = (pWriteSharedMemoryFUNC)lpfnWriteSharedMemory;
				ReadSharedMemory = (pReadSharedMemoryFUNC)lpfnReadSharedMemory;
				DestroySharedMemory = (pDestroySharedMemoryFUNC)lpfnDestroySharedMemory;
				CreateSharedMemory = (pCreateSharedMemoryFUNC)lpfnCreateSharedMemory;
			//}
		break;
...
//		wcout << "Try to CreateLocalMessageBroadcast with " << pLocalMessageBroadcastPartner->sharedMemoryName << "\n";
		...
		if (lpfnLockSharedMemory == NULL 
			||lpfnUnlockSharedMemory == NULL
			|| lpfnWriteSharedMemory == NULL
			|| lpfnReadSharedMemory == NULL
			|| lpfnDestroySharedMemory == NULL
			|| lpfnCreateSharedMemory == NULL
			) {
			wcout << "PROBLEM ============= Null Function Pointers";	
		}
		...
		//pLocalMessageBroadcastPartner->hSharedMemory = CreateSharedMemory( lpSharedMemoryName, 3, 7 );
		
		wcout << "hSharedMemory = " << pLocalMessageBroadcastPartner->hSharedMemory << "\n";
		if (pLocalMessageBroadcastPartner->hSharedMemory != NULL) {
			wcout << "hSharedMemory != NULL but " << pLocalMessageBroadcastPartner->hSharedMemory << "\n";
		}
